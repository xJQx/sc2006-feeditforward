...
@router.get("/leftover-foods", response_model=list[leftover_food_schemas.LeftoverFood])
def get_all_leftover_foods(skip: int = 0,
                  limit: int = 100,
                  db: Session = Depends(get_db)):
    return LeftoverFoodController.getAllLeftoverFoods(db, skip, limit)

@router.get("/leftover-foods/available", response_model=list[leftover_food_schemas.LeftoverFood])
def get_all_available_leftover_foods(skip: int = 0,
                  limit: int = 100,
                  db: Session = Depends(get_db)):
    return LeftoverFoodController.getAllAvailableLeftoverFoods(db, skip, limit)

@router.get('/leftover-food/{leftover_food_id}', response_model=leftover_food_schemas.LeftoverFood)
...
@router.get('/leftover-foods/hawkerid/{hawker_id}', response_model=list[leftover_food_schemas.LeftoverFood])
...
@router.post("/leftover-foods/new", response_model=leftover_food_schemas.LeftoverFood)
...
@router.put("/leftover-foods/update", response_model=leftover_food_schemas.LeftoverFood)
