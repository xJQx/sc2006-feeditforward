...
@router.get("/pickup-jobs", response_model=list[pickup_job_schemas.PickupJob])
def get_all_pickup_jobs(skip: int = 0,
                  limit: int = 100,
                  db: Session = Depends(get_db)):
    return PickupJobController.getAllPickupJobs(db, skip, limit)

@router.get("/pickup-jobs/available", response_model=list[pickup_job_schemas.PickupJob])
def get_all_available_pickup_jobs(skip: int = 0,
                  limit: int = 100,
                  db: Session = Depends(get_db)):
    return PickupJobController.getAllAvailablePickupJobs(db, skip, limit)

@router.get('/pickup-job/{pickup_job_id}', response_model=pickup_job_schemas.PickupJob)
...
@router.get('/pickup-jobs/driverid/{driver_id}', response_model=list[pickup_job_schemas.PickupJob])
...
@router.post("/pickup-jobs/new", response_model=pickup_job_schemas.PickupJob)
...
@router.put("/pickup-jobs/update", response_model=pickup_job_schemas.PickupJob)
