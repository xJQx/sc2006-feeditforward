...
tags_metadata = [
    {
        "name": "PriorityRequest (CRUD)",
        "description": "API CRUD Endpoints for Notification Model"
    },
]

# --------------------------------------------------------------------- #
# -------------------- PriorityRequest (CRUD) ------------------------- #
# --------------------------------------------------------------------- #
@router.get("/priority-requests", response_model=list[priority_request_schemas.PriorityRequest], tags=["PriorityRequest (CRUD)"])
def get_all_priority_requests(skip: int = 0,
                  limit: int = 100,
                  db: Session = Depends(get_db)):
    return PriorityRequestController.getAllPriorityRequests(db, skip, limit)

@router.get('/priority-request/{priority_request_id}', response_model=priority_request_schemas.PriorityRequest, tags=["PriorityRequest (CRUD)"])
...
@router.get('/priority-requests/consumerid/{consumer_id}', response_model=list[priority_request_schemas.PriorityRequest], tags=["PriorityRequest (CRUD)"])
...
@router.get('/priority-requests/status/{status}', response_model=list[priority_request_schemas.PriorityRequest], tags=["PriorityRequest (CRUD)"])
...
@router.post("/priority-request/new", response_model=priority_request_schemas.PriorityRequest, tags=["PriorityRequest (CRUD)"])
...
@router.put("/priority-request/update", response_model=priority_request_schemas.PriorityRequest, tags=["PriorityRequest (CRUD)"])
